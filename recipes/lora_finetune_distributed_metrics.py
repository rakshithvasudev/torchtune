...
class LoRAFinetuneRecipeDistributed(FTRecipeInterface):
    ...
    def __init__(self, cfg: DictConfig) -> None:
        ...
        self._log_system_stats = cfg.get("log_system_stats", False)
    ...
    def get_host_memory_stats(self):
        """
        Get memory used because of this job and total memory in GiB
        """
        memory_info = psutil.virtual_memory()

        used_memory = memory_info.used
        total_memory = memory_info.total
        custom_percentage = (used_memory/total_memory) * 100

        return custom_percentage, total_memory/(1024**3)

    def get_gpu_stats(self):
       # num_gpus = torch.cuda.device_count()
       # total_reserved_memory = sum(torch.cuda.memory_reserved(i) for i in range(num_gpus))
       # average_reserved_memory_gb = total_reserved_memory / num_gpus / (1024 ** 3)
       # gpu_utilization = 
        ...
        for i in range(num_gpus):
            ...
            # Get reserved memory using PyTorch
            total_reserved_memory += torch.cuda.memory_reserved(i)
        
        # Calculate averages
        average_utilization = total_utilization / num_gpus
        average_reserved_memory_gb = total_reserved_memory / num_gpus / (1024 ** 3)

        return average_reserved_memory_gb, average_utilization



    def get_system_stats(self):

        cpu_utilization_percentage = psutil.cpu_percent(interval=None, percpu=False)
        memory_utilization_percentage, total_memory = self.get_host_memory_stats()
        #average_reserved_memory_gb, average_utilization = self.get_gpu_stats()

        
        return {
            'average_cpu_utilization_percentage': round(cpu_utilization_percentage, 2),
            'host_memory_utilization_percentage': round(memory_utilization_percentage, 2),
            #'total_host_memory_gb': round(total_memory, 2),
            #'average_gpu_reserved_memory_gb': round(average_reserved_memory_gb, 2),
            #'average_gpu_utilization_percentage': round(average_utilization, 2)
        }
            ...
            for idx, batch in enumerate(self._dataloader):
                ...
                if (idx + 1) % self._gradient_accumulation_steps == 0:
                    ...
                    if (
                        ...
                        if self._log_system_stats: 
                            system_stats = self.get_system_stats()
                            log_dict.update(system_stats)
